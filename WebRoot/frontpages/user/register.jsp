...
<html>
  <head>
	...
	-->
	<link rel="stylesheet" type="text/css" href="<c:url value='/frontpages/css/user/regist.css'/>">

	<script type="text/javascript" src="<c:url value='/jquery/jquery-1.5.1.js'/>"></script>
	<script type="text/javascript" src="<c:url value='/frontpages/js/user/regist.js'/>"></script>
	<script>
		//换一张验证码
	$(function(){
		$("#huanyizhang").click(function(event){
			event.preventDefault();
			$("#imgVerifyCode").attr("src", "<c:url value='/VerifyCodeServlet?a='/>" + new Date().getTime());
		});
	});
...
<form action="<c:url value='/UserServlet'/>" method="post" id="registForm">
    ...
    <table id="tableForm">
      ...
      <tr>
        <td></td>
        <td>
          <span id="divVerifyCode"><img id="imgVerifyCode" src="<c:url value='/VerifyCodeServlet'/>"/></span>
          <label><a href="javascript:;" id="huanyizhang">换一张</a></label>
        </td>
        <td>
        </td>
      </tr>
